def save_preset_dialog(self):
    """
    Displays a dialog for saving a workflow preset
    """
    try:
        import tkinter as tk
        from tkinter import simpledialog, messagebox
        
        preset_name = simpledialog.askstring("Save Preset", "Enter a name for this preset:", parent=self.root if hasattr(self, 'root') else None)
        if preset_name:
            if hasattr(self, '_save_preset'):
                self._save_preset(preset_name)
            else:
                # Fallback save implementation
                print(f"Saving preset '{preset_name}' (fallback implementation)")
                messagebox.showinfo("Save Preset", f"Preset '{preset_name}' saved successfully")
            return True
        return False
    except Exception as e:
        print(f"Error in save_preset_dialog: {str(e)}")
        import traceback
        traceback.print_exc()
        return False

# ... existing patch_workflow_gui function ...

def patch_workflow_gui(gui_class):
    """Patch the WorkflowGUI class with missing methods"""
    # ... existing code ...
    
    # Add save_preset_dialog method
    gui_class.save_preset_dialog = save_preset_dialog
    
    # Continue with existing patching
    gui_class._save_preset = _save_preset_enhanced if '_save_preset_enhanced' in globals() else _save_preset
    
    # ... rest of the existing patching code ...
    ...
    print("WorkflowGUI patched with missing methods, including save_preset_dialog")
    return gui_class

# Import necessary modules for HPC functionality
import os
import json
import subprocess
import tkinter as tk
...
try:
    ...
    # Define dummy classes/functions if HPC is not available
    class HPCConnector:
        def __init__(self, config=None): self.connected = False; self.config = config
        def connect(self, **kwargs): return False, "HPC modules not available"
...
def setup_hpc_widgets(self):
    ...
    self.hpc_port = ttk.Entry(connection_frame, width=6)
    ...
    auth_frame.grid(row=3, column=0, columnspan=2, sticky='ew', padx=5, pady=5)

    self.auth_type = tk.StringVar(value="password") # Default to password auth
    ttk.Radiobutton(auth_frame, text="Password", variable=self.auth_type, value="password", command=self.toggle_auth_type).pack(side='left', padx=5)
    ttk.Radiobutton(auth_frame, text="Key File", variable=self.auth_type, value="key", command=self.toggle_auth_type).pack(side='left', padx=5)

    # Password authentication
    self.password_frame = ttk.Frame(auth_frame)
    self.password_frame.pack(side='top', fill='x', pady=5)
    ...
    self.hpc_password.pack(side='left', fill='x', expand=True, padx=5)
    ...
    self.hpc_key_path.pack(side='left', fill='x', expand=True, padx=5)
    ttk.Button(self.key_frame, text="Browse...", command=self.browse_key_file).pack(side='right', padx=5)
    ...
    self.job_nodes = ttk.Spinbox(job_frame, from_=1, to=99, width=5)
    ...
    ttk.Label(job_frame, text="Walltime (hh:mm:ss):").grid(row=3, column=0, sticky='w', padx=5, pady=2)
    self.job_walltime = ttk.Entry(job_frame, width=10)
    self.job_walltime.insert(0, "01:00:00")
    self.job_walltime.grid(row=3, column=1, sticky='w', padx=5, pady=2)

    ttk.Label(job_frame, text="Queue/Partition:").grid(row=4, column=0, sticky='w', padx=5, pady=2)
    self.job_queue = ttk.Combobox(job_frame, width=15)
    self.job_queue.grid(row=4, column=1, sticky='w', padx=5, pady=2)

    self.submit_job_button = ttk.Button(job_frame, text="Submit Job", command=self.submit_remote_job, state=tk.DISABLED)
    self.submit_job_button.grid(row=5, column=0, columnspan=2, pady=10)

    # --- Job Monitoring Frame ---
    jobs_frame = ttk.LabelFrame(self.remote_tab, text="Job Monitoring")
    jobs_frame.pack(pady=10, padx=10, fill='both', expand=True)

    # Toolbar for jobs list
    jobs_toolbar = ttk.Frame(jobs_frame)
    jobs_toolbar.pack(fill='x', pady=5)

    self.refresh_jobs_button = ttk.Button(jobs_toolbar, text="Refresh", command=self.refresh_jobs_list, state=tk.DISABLED)
    self.refresh_jobs_button.pack(side='left', padx=5)

    # Jobs treeview
    columns = ('id', 'name', 'status', 'submit_time', 'duration')
    ...
    self.jobs_tree.heading('submit_time', text='Submit Time')
    self.jobs_tree.heading('duration', text='Duration')
    
    self.jobs_tree.column('id', width=80, anchor='center')
    self.jobs_tree.column('name', width=150)
    self.jobs_tree.column('status', width=100, anchor='center')
    self.jobs_tree.column('submit_time', width=120, anchor='center')
    self.jobs_tree.column('duration', width=80, anchor='center')
    
    jobs_scroll = ttk.Scrollbar(jobs_frame, command=self.jobs_tree.yview)
    ...
    # Job actions frame
    job_action_frame = ttk.Frame(jobs_frame)
    job_action_frame.pack(fill='x', pady=5)

    self.cancel_job_button = ttk.Button(job_action_frame, text="Cancel Job", command=self.cancel_remote_job, state=tk.DISABLED)
    self.cancel_job_button.pack(side='left', padx=5)

    self.view_output_button = ttk.Button(job_action_frame, text="View Output", command=self.view_job_output, state=tk.DISABLED)
    self.view_output_button.pack(side='left', padx=5)

    # Setup configuration buttons at the bottom
    config_frame = ttk.Frame(self.remote_tab)
    config_frame.pack(fill='x', pady=10, padx=10)

    self.load_settings_button = ttk.Button(config_frame, text="Load Settings", command=self.load_hpc_profiles)
    self.load_settings_button.pack(side='left', padx=5)
    
    self.save_settings_button = ttk.Button(config_frame, text="Save Settings", command=self.save_hpc_profiles)
    ...
    # Apply initial auth type (default is password)
    self.toggle_auth_type() 

    print("HPC widgets setup complete.")

# ... rest of the HPC functions (toggle_auth_type, browse_key_file, etc.) ...
...
def patch_workflow_gui(gui_class):
    ...
    # Add HPC setup method
    gui_class.setup_hpc_widgets = setup_hpc_widgets
    
    # Add HPC connection methods
    ...
    gui_class._update_jobs_list_ui = _update_jobs_list_ui
    gui_class.refresh_jobs_list = refresh_jobs_list
    
    # Add job action methods
    gui_class.cancel_remote_job = cancel_remote_job
    gui_class.view_job_output = view_job_output
    gui_class._update_text_content = _update_text_content
    gui_class.refresh_output_content = refresh_output_content
    gui_class.fetch_and_update_content = fetch_and_update_content
    
    # Initialize HPC setup when GUI is created
    original_init = gui_class.__init__
    
    def patched_init(self, *args, **kwargs):
        original_init(self, *args, **kwargs)
        # Setup HPC tab and widgets after main GUI is initialized
...
try:
    ...
    class HPCConnector:
        ...
        def connect(self, **kwargs): 
            print("HPC modules not available")
            return False, "HPC modules not available"
        def disconnect(self): pass
        def execute_command(self, command, timeout=30): raise NotImplementedError("HPC modules not available")
        def upload_file(self, local_path, remote_path): raise NotImplementedError("HPC modules not available")
        def download_file(self, remote_path, local_path): raise NotImplementedError("HPC modules not available")
...
def toggle_auth_type(self):
    ...
    if not hasattr(self, 'auth_type'):
        print("Error: auth_type not initialized")
        return
    
    auth_type = self.auth_type.get()
    if auth_type == "password":
        if hasattr(self, 'password_frame'): self.password_frame.pack(side='top', fill='x', pady=5)
        if hasattr(self, 'key_frame'): self.key_frame.pack_forget()
    else:  # key
        if hasattr(self, 'password_frame'): self.password_frame.pack_forget()
        if hasattr(self, 'key_frame'): self.key_frame.pack(side='top', fill='x', pady=5)

def browse_key_file(self):
    """Open file dialog to select SSH private key file."""
    filename = filedialog.askopenfilename(
        title="Select SSH Private Key", 
        filetypes=(("All files", "*.*"), ("PEM files", "*.pem"), ("OpenSSH files", "*.key"))
    )
    if filename and hasattr(self, 'hpc_key_path'):
...
def get_hpc_config(self):
    ...
    try:
        ...
        if config["use_key"]:
            key_path = self.hpc_key_path.get().strip()
            if not key_path:
            ...
            password = self.hpc_password.get()
            if not password:
                raise ValueError("Password is required for password authentication")
            config["password"] = password
            config["key_path"] = None  # Ensure key path is not used
        ...
        messagebox.showerror("Error", f"Failed to get HPC configuration: {str(e)}")
        traceback.print_exc()
        return None
...
def _update_connection_result(self, success, message):
    ...
    if not hasattr(self, 'connection_status_var') or not hasattr(self, 'connection_status_label'):
        print("Warning: connection_status widgets not initialized.")
        return

    if success:
        self.connection_status_var.set(f"Status: Connected")
        self.connection_status_label.config(foreground="green")
        # Enable job submission and refresh buttons when connected
        if hasattr(self, 'submit_job_button'): self.submit_job_button.config(state=tk.NORMAL)
        if hasattr(self, 'refresh_jobs_button'): self.refresh_jobs_button.config(state=tk.NORMAL)
    else:
        self.connection_status_var.set(f"Status: Failed - {message}")
        self.connection_status_label.config(foreground="red")
        # Disable job submission and refresh buttons when not connected
        if hasattr(self, 'submit_job_button'): self.submit_job_button.config(state=tk.DISABLED)
    ...
    if hasattr(self, 'test_connection_button'): self.test_connection_button.config(state=tk.NORMAL)

def _update_cluster_info_thread(self):
    """Worker thread to fetch and update cluster info."""
    if not hasattr(self, 'hpc_connector') or not self.hpc_connector or not self.hpc_connector.connected:
        print("Cannot update cluster info: Not connected to HPC")
        return

    try:
        cluster_info = self.hpc_connector.get_cluster_info()
        # Also fetch partitions/queues for job submission
        partitions = self.hpc_connector.get_partitions() if hasattr(self.hpc_connector, 'get_partitions') else []
        
        # Schedule UI update on the main thread
        if hasattr(self, 'root'):
            self.root.after(0, lambda: self._update_cluster_info(cluster_info, partitions))
    except Exception as e:
        print(f"Error fetching cluster info: {e}")
        traceback.print_exc()

def _update_cluster_info(self, info, partitions=None):
    """Update the UI text widget with cluster information."""
    if not hasattr(self, 'hpc_info_text'):
        print("Warning: hpc_info_text not initialized.")
        return

    # Update the text widget with cluster info
    ...
    if isinstance(info, dict):
        for key, value in info.items():
            if key != "error":  # Skip error messages
                self.hpc_info_text.insert(tk.END, f"{key}: {value}\n")
    else:
        self.hpc_info_text.insert(tk.END, str(info))
    
    self.hpc_info_text.config(state=tk.DISABLED)
    
    # Update partition/queue dropdown if available
    if partitions and hasattr(self, 'job_queue'):
        self.job_queue['values'] = partitions
        if partitions:
    ...
    try:
        ...
        if success:
            ...
            if not self.hpc_connector.connected:
                 ...
                 # Start background thread to update cluster info
...
def get_hpc_profiles(self):
    ...
    if hasattr(self, 'job_cores'): settings["job_cores"] = self.job_cores.get()
...
def set_hpc_profiles(self, settings):
    ...
    if hasattr(self, 'job_cores'): self.job_cores.delete(0, tk.END); self.job_cores.insert(0, settings.get("job_cores", "4"))
    ...
    if hasattr(self, 'job_queue'): self.job_queue.delete(0, tk.END); self.job_queue.insert(0, settings.get("job_queue", ""))
...
def refresh_queue_list(self):
    ...
    if not self.hpc_connector or not self.hpc_connector.connected:
        print("Cannot refresh queues: Not connected to HPC")
        return

    try:
        # This depends on your connector implementation
        if hasattr(self.hpc_connector, 'get_partitions'):
            partitions = self.hpc_connector.get_partitions()
            if hasattr(self, 'job_queue') and partitions:
                self.job_queue['values'] = partitions
                print(f"Updated queue list with {len(partitions)} partitions")
    except Exception as e:
...
def load_hpc_profiles(self):
    ...
    if os.path.exists(settings_file):
        ...
        messagebox.showinfo("Load Settings", "No saved HPC settings found.")

def create_job_script(self):
    """Creates a job submission script based on GUI parameters."""
    if not self.hpc_connector or not self.hpc_connector.connected:
        messagebox.showerror("Error", "Not connected to HPC. Please connect first.")
        return None
    ...
    try:
        ...
        # Collect job parameters from GUI
        job_params = {
            "name": job_name,
            "nodes": int(self.job_nodes.get()),
            "cores_per_node": int(self.job_cores.get()),
            "walltime": self.job_walltime.get(),
            "queue": self.job_queue.get(),
            "scheduler": self.hpc_scheduler.get()
        }

        # Create the script using HPCConnector
        script = self.hpc_connector.create_job_script(job_params)
        return script
        ...
        messagebox.showerror("Error", f"Failed to create job script: {str(e)}")
        traceback.print_exc()
        return None
...
def show_job_script_confirmation(self, script_content, job_name):
    ...
    confirm_window.title(f"Job Script Preview - {job_name}")
    confirm_window.geometry("600x500")
    confirm_window.grab_set()  # Make the window modal

    # Script preview
    preview_frame = ttk.LabelFrame(confirm_window, text="Job Script Preview")
    preview_frame.pack(padx=10, pady=10, fill=tk.BOTH, expand=True)

    script_text = tk.Text(preview_frame, wrap=tk.WORD)
    script_scroll = ttk.Scrollbar(preview_frame, command=script_text.yview)
    script_text.config(yscrollcommand=script_scroll.set)
    ...
    script_text.config(state=tk.DISABLED)  # Make it read-only

    # Button frame
    button_frame = ttk.Frame(confirm_window)
    button_frame.pack(pady=10, padx=10, fill=tk.X)

    result = [False]  # Using a list to allow modification from inner function
    ...
    submit_button = ttk.Button(button_frame, text="Submit Job", command=on_submit)
...
def _submit_job_thread(self, job_script, job_name):
    ...
    try:
        ...
        if job:
            if hasattr(self, 'root'):
                self.root.after(0, lambda: self._add_job_to_list(job))
            print(f"Job submitted: {job.id} - {job.name}")
        else:
            if hasattr(self, 'root'):
                self.root.after(0, lambda: messagebox.showerror("Job Submission Failed", "Failed to submit job. See console for details."))
            print("Job submission failed: No job object returned")
            
    except Exception as e:
        if hasattr(self, 'root'):
            self.root.after(0, lambda: messagebox.showerror("Error", f"Job submission error: {str(e)}"))
...
def submit_remote_job(self):
    ...
    if not job_script:
        return None  # Error already shown by create_job_script

    # Get job name for display in confirmation
    job_name = self.job_name.get().strip()
    
    # Show confirmation dialog with script preview
    if self.show_job_script_confirmation(job_script, job_name):
...
def _add_job_to_list(self, job):
    ...
    # Store job in active jobs dictionary
    self.active_jobs[job.id] = job
    
    # Add to treeview at the top (insert at beginning)
    self.jobs_tree.insert('', 'end', values=(
        job.id, job.name, job.status, 
        job.submit_time if hasattr(job, 'submit_time') else "just now",
        job.duration if hasattr(job, 'duration') else "0:00"
    ))
...
def _update_job_in_list(self, job_id, status_name, duration):
    ...
    for item in self.jobs_tree.get_children():
        if self.jobs_tree.item(item)['values'][0] == job_id:  # Check the ID column
            values = list(self.jobs_tree.item(item)['values'])  # Convert tuple to list
            values[2] = status_name  # Update status
            values[4] = duration  # Update duration
...
def _monitor_job_status_thread(self, job_id):
    ...
    if not self.hpc_connector or not self.hpc_connector.connected:
        print(f"Cannot monitor job {job_id}: Not connected to HPC")
        return
        
    try:
        max_iterations = 180  # Limit monitoring time (e.g., 30 minutes at 10s intervals)
        iteration = 0
        
        job_complete = False
        while not job_complete and iteration < max_iterations:
            time.sleep(10)  # Check every 10 seconds
            iteration += 1
            
            if not self.hpc_connector or not self.hpc_connector.connected:
                print(f"Lost HPC connection while monitoring job {job_id}")
                break
                
            # Get updated job info
            job_status = self.hpc_connector.get_job_status(job_id)
            if not job_status:
                print(f"Failed to get status for job {job_id}")
                continue
                
            status_name = job_status.status if hasattr(job_status, 'status') else "UNKNOWN"
            duration = job_status.duration if hasattr(job_status, 'duration') else "N/A"
            
            # Update UI in main thread
            if hasattr(self, 'root'):
                self.root.after(0, lambda: self._update_job_in_list(job_id, status_name, duration))
            
            # Update active jobs record
            if job_id in self.active_jobs:
                self.active_jobs[job_id].status = status_name
                if hasattr(job_status, 'duration'):
                    self.active_jobs[job_id].duration = duration
            
            # Check if job is complete
            if status_name in [HPCJobStatus.COMPLETED, HPCJobStatus.FAILED, HPCJobStatus.CANCELLED]:
                job_complete = True
                print(f"Job {job_id} monitoring complete: Status = {status_name}")
...
def monitor_job_status(self, job_id):
    ...
    threading.Thread(target=self._monitor_job_status_thread, args=(job_id,), daemon=True).start()
...
def _on_job_select(self, event):
    ...
    if selected_items:  # If something is selected
        # Enable buttons
        if hasattr(self, 'cancel_job_button'): self.cancel_job_button.config(state=tk.NORMAL)
        if hasattr(self, 'view_output_button'): self.view_output_button.config(state=tk.NORMAL)
    else:
        # Disable buttons if nothing selected
        if hasattr(self, 'cancel_job_button'): self.cancel_job_button.config(state=tk.DISABLED)
        if hasattr(self, 'view_output_button'): self.view_output_button.config(state=tk.DISABLED)

def _refresh_jobs_list_thread(self):
    """Background thread for refreshing the job list."""
    if not self.hpc_connector or not self.hpc_connector.connected:
        if hasattr(self, 'root'): 
            self.root.after(0, lambda: messagebox.showwarning("Warning", "Not connected to HPC."))
        if hasattr(self, 'refresh_jobs_button'):
            self.root.after(0, lambda: self.refresh_jobs_button.config(state=tk.NORMAL))
        return

    try:
        # Get current jobs from HPC
        jobs = self.hpc_connector.get_jobs()
        
        # Update UI in main thread
        if hasattr(self, 'root'):
            self.root.after(0, lambda: self._update_jobs_list_ui(jobs))
        
    except Exception as e:
        if hasattr(self, 'root'):
            self.root.after(0, lambda: messagebox.showerror("Error", f"Failed to refresh job list: {str(e)}"))
        print(f"Error refreshing jobs list: {e}")
        traceback.print_exc()
    finally:
        # Re-enable refresh button
        if hasattr(self, 'root') and hasattr(self, 'refresh_jobs_button'):
            self.root.after(0, lambda: self.refresh_jobs_button.config(state=tk.NORMAL))

def _update_jobs_list_ui(self, jobs):
    """Update the job list UI with the fetched jobs."""
    if not hasattr(self, 'jobs_tree'): return
    
    # Clear current list
    self.jobs_tree.delete(*self.jobs_tree.get_children())
    
    # Update active jobs dictionary
    self.active_jobs = {job.id: job for job in jobs}
    ...
    for job in jobs:
        try:
            self.jobs_tree.insert('', 'end', values=(
                job.id,
                job.name if hasattr(job, 'name') else "unknown",
                job.status if hasattr(job, 'status') else "UNKNOWN",
                job.submit_time if hasattr(job, 'submit_time') else "N/A",
                job.duration if hasattr(job, 'duration') else "N/A"
            ))
            
            # Start monitoring if job is active
            if hasattr(job, 'status') and job.status in [HPCJobStatus.PENDING, HPCJobStatus.RUNNING]:
                self.monitor_job_status(job.id)
        except Exception as e:
            print(f"Error adding job {job.id} to list: {e}")
            continue
...
def cancel_remote_job(self):
    """Cancels the selected job on the HPC system."""
    if not self.hpc_connector or not self.hpc_connector.connected:
        messagebox.showwarning("Warning", "Not connected to HPC.")
        return
    ...
    if not selected_items:
        messagebox.showinfo("Info", "No job selected to cancel.")
        return

    job_id = self.jobs_tree.item(selected_items[0])['values'][0]  # Get ID from first column
    job_name = self.jobs_tree.item(selected_items[0])['values'][1]  # Get name from second column
    
    if messagebox.askyesno("Confirm Cancel", f"Are you sure you want to cancel job {job_name} (ID: {job_id})?"):
        try:
            result = self.hpc_connector.cancel_job(job_id)
            if result:
                messagebox.showinfo("Success", f"Job {job_id} cancelled successfully.")
                # Update job status in UI
                self._update_job_in_list(job_id, HPCJobStatus.CANCELLED, "N/A")
                # Update in our records
                if job_id in self.active_jobs:
                    self.active_jobs[job_id].status = HPCJobStatus.CANCELLED
            else:
                messagebox.showwarning("Warning", f"Failed to cancel job {job_id}.")
        except Exception as e:
            messagebox.showerror("Error", f"Error cancelling job: {str(e)}")
            traceback.print_exc()

def view_job_output(self):
    """View output files for the selected job."""
    if not self.hpc_connector or not self.hpc_connector.connected:
        messagebox.showwarning("Warning", "Not connected to HPC.")
        return
    ...
    selected_items = self.jobs_tree.selection()
    if not selected_items:
        messagebox.showinfo("Info", "No job selected to view.")
        return

    job_id = self.jobs_tree.item(selected_items[0])['values'][0]  # Get ID from first column
    job_name = self.jobs_tree.item(selected_items[0])['values'][1]  # Get name from second column
    
    # Open output viewer in a new window
    output_window = tk.Toplevel(self.root)
    output_window.title(f"Job Output - {job_name} ({job_id})")
    output_window.geometry("700x500")
    
    # Create notebook for different output files
    output_notebook = ttk.Notebook(output_window)
    output_notebook.pack(fill='both', expand=True, padx=10, pady=10)
    
    # Get output file paths - customize based on your system
    output_paths = []
    try:
        # This implementation will depend on your HPC system's structure
        if job_id in self.active_jobs and hasattr(self.active_jobs[job_id], 'remote_dir'):
            remote_dir = self.active_jobs[job_id].remote_dir
            output_paths = [
                (f"{job_id}.out", f"{remote_dir}/{job_id}.out"),
                (f"{job_id}.err", f"{remote_dir}/{job_id}.err"),
                ("job_script.sh", f"{remote_dir}/job_script.sh")
            ]
        else:
            # fallback if we don't have remote_dir info
            output_paths = [
                (f"{job_id}.out", f"~/{job_id}.out"),
                (f"{job_id}.err", f"~/{job_id}.err")
            ]
    except Exception as e:
        messagebox.showerror("Error", f"Error determining output file paths: {str(e)}")
        return
    
    # Create a tab for each output file
    for label, path in output_paths:
        try:
            # Create tab
            tab = ttk.Frame(output_notebook)
            output_notebook.add(tab, text=label)
            
            # Create text widget with scrollbar
            text_widget = tk.Text(tab, wrap=tk.WORD)
            scroll = ttk.Scrollbar(tab, command=text_widget.yview)
            ...
            # Start a thread to fetch the actual content
            def fetch_content(text_widget, path):
                try:
                    content = self.hpc_connector.get_file_content(path)
                    # Update UI in main thread
                    if hasattr(self, 'root'):
                        self.root.after(0, lambda: self._update_text_content(text_widget, content))
                except Exception as e:
                    error_msg = f"Failed to load file: {str(e)}"
                    if hasattr(self, 'root'):
                        self.root.after(0, lambda: self._update_text_content(text_widget, error_msg))
            
            threading.Thread(target=fetch_content, args=(text_widget, path), daemon=True).start()
            
        except Exception as e:
            print(f"Error setting up output tab for {label}: {e}")
            continue
    
    # Add refresh button at the bottom
    button_frame = ttk.Frame(output_window)
    button_frame.pack(fill='x', padx=10, pady=10)
    ttk.Button(button_frame, text="Refresh", command=lambda: self.refresh_output_content(output_notebook)).pack(side='left')
    ttk.Button(button_frame, text="Close", command=output_window.destroy).pack(side='right')

def _update_text_content(self, text_widget, content):
...
def refresh_output_content(self, notebook):
    """Refresh the content of all output tabs."""
    # For each tab in the notebook, refresh its content
    for tab_id in notebook.tabs():
        # Get the text widget in this tab
        tab = notebook.nametowidget(tab_id)
        for widget in tab.winfo_children():
            if isinstance(widget, tk.Text):
                text_widget = widget
                
                # Get tab label and path information - you might need to store this separately
                tab_index = notebook.index(tab_id)
                tab_text = notebook.tab(tab_id, "text")
                
                # Determine path based on tab text - customize for your system
                path = ""
                if tab_text.endswith('.out'):
                    path = f"~/{tab_text}"
                elif tab_text.endswith('.err'):
                    path = f"~/{tab_text}"
                else:
                    path = f"~/{tab_text}"
                    
                # Start a thread to fetch updated content
                threading.Thread(target=lambda: self.fetch_and_update_content(text_widget, path), daemon=True).start()

def fetch_and_update_content(self, text_widget, path):
    """Fetch file content and update text widget."""
    try:
        content = self.hpc_connector.get_file_content(path)
        # Update in UI thread
        if hasattr(self, 'root'):
        ...
        error_msg = f"Failed to refresh content: {str(e)}"
        if hasattr(self, 'root'):
            self.root.after(0, lambda: self._update_text_content(text_widget, error_msg))
...
def patch_workflow_gui(gui_class):
    ...
    return gui_class
def _download_complete(self, job_id, remote_dir, local_dir):
    """Callback function to handle successful completion of the download process."""
    try:
        self.update_status("Download completed successfully", show_progress=False)
        messagebox.showinfo("Download Complete", 
                           f"Job {job_id} results have been successfully downloaded to {local_dir}")
        ...
        if os.path.exists(local_dir):
            try:
                ...
                self.log(f"Error opening download folder: {str(e)}")
    except Exception as e:
        self.log(f"Error in download completion handler: {str(e)}")

def patch_workflow_gui(gui_class):
    """Patch the WorkflowGUI class with missing methods"""    
    # Make sure _download_complete is included in the patching
    gui_class.load_hpc_profiles = load_hpc_profiles
    gui_class.submit_remote_job = submit_remote_job
    gui_class.show_job_script_confirmation = show_job_script_confirmation
    gui_class.refresh_jobs_list = refresh_jobs_list
    ...
    gui_class._download_complete = _download_complete  # Add the missing function
...
def _download_error(self, error_message):
    """Callback function to handle errors during the download process."""
    try:
        # Update the status to indicate an error
        self.update_status(f"Download error: {error_message}")

        # Optionally, display a message box to the user
        messagebox.showerror("Download Error", f"An error occurred during the download: {error_message}")

        # Log the error
        self.log(f"Download error: {error_message}")
    except Exception as e:
        # Handle any unexpected errors
        print(f"Error in _download_error: {str(e)}")
    """Callback function to handle errors during the download process."""
    try:
        # Update the status to indicate an error
        self.update_status(f"Download error: {error_message}")
        ...
        # Handle any unexpected errors
        print(f"Error in _download_error: {str(e)}")
    """Callback function to handle the completion of a download operation."""
    try:
        # Update the status to indicate completion
        self.update_status(f"Download complete. Files saved to: {local_dir}")

        # Optionally, display a message box to the user
        messagebox.showinfo("Download Complete", f"Files have been successfully downloaded to: {local_dir}")

        # Log the completion
        self.log(f"Download completed successfully. Files saved to: {local_dir}")
    except Exception as e:
        # Handle any unexpected errors
        self.update_status(f"Error in download completion: {str(e)}")
        print(f"Error in _download_complete: {str(e)}")
    try:
        # Update the status to indicate completion
        self.update_status(f"Download complete. Files saved to: {local_dir}")
...
def patch_workflow_gui(gui_class):
    ...
    gui_class._download_error = _download_error
    gui_class.update_connection_status = _update_connection_result
